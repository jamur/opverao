...
from .models import Estagiario, AvaliacaoDeEstagiario, Curso
import csv
from django.http import HttpResponse
from datetime import datetime, timedelta

def index(request):
    return render(request, 'cadestagiarios/index.html')

def valores(request):
    estagiarios = Estagiario.objects.all()
    return render(request, 'cadestagiarios/valores.html', {'es': estagiarios})

def nomes(request):
    estagiarios = Estagiario.objects.all().order_by('nome_do_estagiario')
    return render(request, 'cadestagiarios/nomes.html', {'estagiarios': estagiarios})

def telefones(request):
    estagiarios = Estagiario.objects.all().order_by('nome_do_estagiario')
    return render(request, 'cadestagiarios/nomes.html', {'estagiarios': estagiarios,'com_telefones':True})
    
def falta_pagamento(request):
    estagiarios = Estagiario.objects.filter(falta_pagamento__exact=True).order_by('nome_do_estagiario')
    return render(request, 'cadestagiarios/falta_pagamento.html', {'estagiarios': estagiarios})

def falta_pagamento_texto(request):
    estagiarios = Estagiario.objects.filter(falta_pagamento__exact=True).order_by('nome_do_estagiario')
    return render(request, 'cadestagiarios/falta_pagamento_texto.html', {'estagiarios': estagiarios})

def falta_pagamento_texto_so_valor(request):
    estagiarios = Estagiario.objects.filter(falta_pagamento__exact=True).order_by('nome_do_estagiario')
    return render(request, 'cadestagiarios/falta_pagamento_texto_so_valor.html', {'estagiarios': estagiarios})
